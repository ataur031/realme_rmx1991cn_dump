...
function collect_android_log()
{
    ...
    dd if=dev/zero of=sdcard/zero2 bs=1m count=2
    dd if=dev/zero of=sdcard/zero10 bs=1m count=10
    dd if=sdcard/zero10 of=dev/block/by-name/opporeserve3
    dd if=sdcard/zero2 of=dev/block/by-name/opporeserve3 bs=1m seek=${SYSTEM_SERVER_OFFSET}
    rm -rf sdcard/zero10
    rm -rf sdcard/zero2
