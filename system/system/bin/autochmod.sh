...
function DumpEnvironment(){
    ...
    #ifdef VENDOR_EDIT
    #yanghao@PSW.Kernel.Stability, 2019/7/6, Add for record the slab meminfo
    cat /proc/slabinfo > /cache/environment/slabinfo.txt &
    cat /proc/mtk_memcfg/slabtrace > /cache/environment/slabtrace.txt &
    #endif VENDOR_EDIT
...
function tranfer2TfCard(){
    ...
    cp -rf sdcard/Coloros/os/Browser/.log/* ${newpath}/os/Browser/
...
if [ "$devinfo" == "17085" ] || [ "$devinfo" == "18383" ]; then
    ...
    chown -R system:system /data/oppo/log/opporeserve/media
